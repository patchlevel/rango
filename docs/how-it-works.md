...
* **Advanced aggregation expressions**: arithmetic, string, comparison, boolean, conditional, and date operators are supported (see [aggregation](aggregation.md)), but array operators, `$map`/`$reduce`/`$filter`, `$facet`, and window functions are not
